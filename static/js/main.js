...
<div id="loginSection" class="login-container">
  <div class="login-card">
    ...
    <form id="loginForm">
      <div class="mb-3">
        <input type="text" class="form-control" id="loginUsername" placeholder="Nom d'utilisateur" required>
      </div>
      <div class="mb-3">
            ...
            <div class="form-section" id="userInfoSection">
              ...
              <div class="row g-3">
                ...
                <div class="col-md-6">
                  ...
                  <input type="text" class="form-control" name="direction" id="inputDirection" placeholder="ex: DSI" required>
                </div>
                <div class="col-md-6">
            ...
            <div class="agreement-section" id="agreementSection">
              ...
              <div class="row">
                <div class="col-md-6">
                  ...
                  <p><strong>Direction :</strong> <span id="dirRemise"></span></p>
...
<script>
  // Global variables - MOVED TO GLOBAL SCOPE
  let users = [
    { username: 'admin', password: 'admin123', role: 'admin', name: 'Administrateur' }
  ];
  let savedRemises = JSON.parse(localStorage.getItem('amendis_remises') || '[]');
  ...
  window.simpleEditUser = function(index) {
  ...
  window.simpleDeleteUser = function(index) {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('loginForm').addEventListener('submit', function(e) {
      ...
      if (user) {
        ...
        document.getElementById('userWelcome').textContent = `Bienvenue, ${user.name}`;
    ...
    function initializeApp() {
      ...
      // Generate unique document number with auto-increment
      function generateDocumentNumber() {
        ...
        while (!isUnique) {
          ...
          newNumber = `REM-${year}-${formattedCounter}`;
          
          // Check if this number already exists in saved remises
          isUnique = !savedRemises.some(remise => remise.id === newNumber);
        }
        
        return newNumber;
      }
      
      // Initialize document number with uniqueness check
      const numero = generateDocumentNumber();
      ...
      function resetForm() {
        ...
        // Generate new document number (but don't increment counter yet)
        const currentYear = new Date().getFullYear();
        let newNumber;
        let testCounter = window.remiseCounter;
        let isUnique = false;
        
        // Find next unique number without permanently incrementing
        while (!isUnique) {
          testCounter = testCounter + 1;
          const formattedCounter = String(testCounter).padStart(5, '0');
          newNumber = `REM-${currentYear}-${formattedCounter}`;
          
          // Check if this number already exists in saved remises
          isUnique = !savedRemises.some(remise => remise.id === newNumber);
        }
        
        document.getElementById("numeroRemise").value = newNumber;
        document.getElementById("refRemise").textContent = newNumber;
      ...
      document.getElementById("saveBtn").addEventListener("click", function() {
        if (validateForm(true)) {
          const formData = {
            id: document.getElementById("numeroRemise").value,
          ...
          // Check for duplicates based on reference number AND serial number
          const duplicateRef = savedRemises.find(r => r.id === formData.id);
          const duplicateSerial = savedRemises.find(r => r.numero_serie.toLowerCase() === formData.numero_serie.toLowerCase());
          
          if (duplicateRef) {
            showMessage(`<strong>Erreur de duplication :</strong><br>Cette référence existe déjà !<br><strong>Référence :</strong> ${formData.id}<br>Veuillez actualiser la page ou cliquer sur "Nouvelle Remise".`, 'error', 0);
            return;
          }
          
          if (duplicateSerial) {
            showMessage(`<strong>Erreur de duplication :</strong><br>Un équipement avec ce numéro de série existe déjà !<br><strong>Numéro de série :</strong> ${formData.numero_serie}<br><strong>Référence existante :</strong> ${duplicateSerial.id}<br><strong>Utilisateur :</strong> ${duplicateSerial.prenom} ${duplicateSerial.nom}`, 'error', 0);
            return;
          }
          
          // Save the data
          savedRemises.push(formData);
          localStorage.setItem('amendis_remises', JSON.stringify(savedRemises));
          
          // Mark form as saved and show indicator
          formSaved = true;
          const fullName = `${formData.prenom} ${formData.nom}`.trim();
          showSavedIndicator(formData.id, fullName);
          ...
          // Show success message
          showMessage(`<strong>Données enregistrées avec succès !</strong><br>Référence : <strong>${formData.id}</strong><br>Utilisateur : <strong>${fullName}</strong>`, 'success');
        } else {
      ...
      function generatePDF(formData) {
        try {
          ...
          doc.setFontSize(8);
          doc.setFont('helvetica', 'bold');
          doc.text(`Référence: ${formData.id}`, margin + 3, y + 4);
          doc.text(`Date: ${new Date(formData.date_creation).toLocaleDateString('fr-FR')}`, pageWidth - margin - 3, y + 4, { align: 'right' });
          ...
          y = addCompactRow("Direction", formData.direction, y);
          ...
          doc.text(`Date: ${new Date(formData.date_creation).toLocaleDateString('fr-FR')}`, margin + 2, y + 11);
          ...
          doc.text(`Date: ${new Date(formData.date_creation).toLocaleDateString('fr-FR')}`, rightBoxX + 2, y + 11);
      ...
      function updateCharts() {
        ...
        const chartColors = {
          primary: '#dc3545',      // Red (IT)
          secondary: '#28a745',    // Green (OT)
          tanger: '#17a2b8',       // Blue (Tanger)
          tetouan: '#ffc107',      // Yellow (Tétouan)
          equipment: '#696969',    // Gray for equipment types
          accent1: '#6f42c1',      // Purple
          accent2: '#fd7e14',      // Orange
          accent3: '#20c997',      // Teal
          accent4: '#e83e8c'       // Pink
        };

        // Clear existing charts before creating new ones
        const existingCharts = Chart.getChart('categoryChart') || Chart.getChart('equipmentChart') || Chart.getChart('locationChart') || Chart.getChart('trendsChart');
        if (existingCharts) {
        ...
        // Generate colors for equipment types
        const equipmentTypes = Object.keys(typeCount);
        const equipmentColors = equipmentTypes.map((_, index) => {
          const colors = [
            chartColors.primary, chartColors.secondary, chartColors.tanger, 
            chartColors.tetouan, chartColors.accent1, chartColors.accent2, 
            chartColors.accent3, chartColors.accent4, chartColors.equipment
          ];
        ...
        new Chart(equipmentCtx, {
          ...
          data: {
            ...
            datasets: [{
              ...
              backgroundColor: equipmentColors,
              borderColor: equipmentColors.map(color => color),
              borderWidth: 1
            }]
          },
          options: {
        ...
        // Location Chart - Using consistent colors
        ...
        // Map locations to consistent colors
        const locations = Object.keys(locationCount);
        const locationColors = locations.map(location => {
          if (location === 'Tanger') return chartColors.tanger;
          if (location === 'Tétouan') return chartColors.tetouan;
          // For any additional locations, use accent colors
          const index = locations.indexOf(location);
          const fallbackColors = [chartColors.accent1, chartColors.accent2, chartColors.accent3, chartColors.accent4];
        ...
        sortedRemises.forEach(remise => {
          ...
          historyCard.innerHTML = `
            <div class="history-meta">
              <span class="history-ref">${remise.id}</span>
              <span class="history-date">${new Date(remise.date_creation).toLocaleDateString('fr-FR')}</span>
            </div>
            <div class="row">
              <div class="col-md-6">
                <p><strong>Utilisateur:</strong> ${remise.prenom} ${remise.nom}</p>
                <p><strong>Direction:</strong> ${remise.direction}</p>
      ...
      function handleReprintPDF(remiseId) {
        ...
        const remise = savedRemises.find(r => r.id === remiseId);
        if (!remise) {
        ...
        const detailsContent = `
          ...
          <div class="detail-section">
            ...
            <div class="detail-row">
              ...
              <div class="detail-value">${new Date(remise.date_creation).toLocaleString('fr-FR')}</div>
            </div>
            <div class="detail-row">
          ...
          <div class="card">
            ...
            <div class="card-body">
              ...
              <div class="row">
                ...
                <div class="col-6">${new Date(remise.date_creation).toLocaleDateString('fr-FR')}</div>
              </div>
            </div>
          </div>
        ...
        if (confirmBtn) {
          ...
          newConfirmBtn.addEventListener('click', function() {
            ...
            if (remiseId) {
              ...
              // Perform deletion
              const index = savedRemises.findIndex(r => r.id === remiseId);
              console.log('Found index:', index);
              
              if (index > -1) {
                const deletedRemise = savedRemises[index];
                savedRemises.splice(index, 1);
                ...
                showJournalMessage(`<strong>✅ Remise supprimée avec succès !</strong><br>Référence: <strong>${deletedRemise.id}</strong><br>Utilisateur: <strong>${deletedRemise.prenom} ${deletedRemise.nom}</strong><br>Les données ont été mises à jour.`, 'success');
              } else {
                showJournalMessage('<strong>Erreur :</strong><br>Impossible de supprimer la remise. Référence non trouvée.', 'error');
              }
            }
          });
        }
      ...
      window.reprintPDF = function(remiseId) {
        ...
        try {
          ...
          doc.text(`Référence: ${remise.id}`, margin + 5, y + 6);
          doc.text(`Date d'émission: ${new Date(remise.date_creation).toLocaleDateString('fr-FR')}`, pageWidth - margin - 5, y + 6, { align: 'right' });
          ...
          const userInfo = [
            ...
            { label: "Direction", value: remise.direction },
          ...
          doc.text(`Date: ${new Date(remise.date_creation).toLocaleDateString('fr-FR')}`, margin + 5, signatureY + 15);
          ...
          doc.text(`Date: ${new Date(remise.date_creation).toLocaleDateString('fr-FR')}`, rightBoxX + 5, signatureY + 15);
